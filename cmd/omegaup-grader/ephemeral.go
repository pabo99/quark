...
func (h *runHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	if len(tokens) == 4 && tokens[2] == "new" && tokens[3] == "" {
		...
		if _, ok := validEphemeralRunFilenames[tokens[3]]; !ok {
			w.WriteHeader(http.StatusNotFound)
			return
		}
		runDirectory, ok := h.ephemeralRunManager.Get(tokens[2])
		if !ok {
			w.WriteHeader(http.StatusNotFound)
			return
		}

		filename := path.Join(runDirectory, tokens[3])
